...
import { getManager, getConnection, getRepository, Repository, createQueryBuilder } from 'typeorm';
import { JwtService } from 'src/jwt/jwt.service';
import { User } from './entities/user.entity';
import { Follow } from './entities/follow.entity';
import { FollowFriendInput, FollowFriendOutput } from './dtos/follow-friend.dto';
...
import { EditPrivateProfileInput, EditPrivateProfileOutput } from './dtos/edit-private-profile.dto';
...
export class UsersService {
    ...
    async createAccount({ email, password }: CreateAccountInput): Promise<CreateAccountOutput> {
        try {
            ...
            const saltRounds = 10;
            const hash = await this.hashPasswordRepository.hashPassword(password, saltRounds);
            const isMatch = await this.hashPasswordRepository.compareHashPassword(password, hash);
            if (!isMatch) {
    ...
    async editPersonalProfile(
        ...
        try {
            ...
            for (const [editProfileInput, inputValue] of Object.entries(editPersonalProfileInput)) {
    ...
    async followFriend(
        ...
        try {
            ...
            const follow = await this.follow.save(
